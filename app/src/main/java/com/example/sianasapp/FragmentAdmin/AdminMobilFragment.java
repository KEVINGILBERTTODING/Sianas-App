...
public class AdminMobilFragment extends Fragment {
    private FragmentAdminMobilBinding binding;

    AnggotaIService anggotaIService;
    LinearLayoutManager linearLayoutManager;
    List<MobilModel> mobilModelList;
    AdminMobilAdapter adminMobilAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentAdminMobilBinding.inflate(inflater, container, false);
        ...
        anggotaIService.getMobilByStatus(status).enqueue(new Callback<List<MobilModel>>() {
            ...
            public void onResponse(Call<List<MobilModel>> call, Response<List<MobilModel>> response) {
                if (response.isSuccessful() && response.body().size() > 0) {
                    ...
                    adminMobilAdapter = new AdminMobilAdapter(getContext(), mobilModelList);
                    linearLayoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false);
                    binding.rvMobil.setLayoutManager(linearLayoutManager);
                    binding.rvMobil.setAdapter(adminMobilAdapter);
